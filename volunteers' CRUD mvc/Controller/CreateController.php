<?php
$name = $email = $phonenumber = $hours = $address = $compensation = "";
$name_err = $email_err = $phonenumber_err = $hours_err = $address_err = $compensation_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_err) && empty($email_err) && empty($phonenumber_err) && empty($hours_err) && empty($address_err) && empty($compensation_err)) {
        include_once '../Model/CreateClass.php';
        $creator = new CreateClass();
        if ($creator->insertRecord($name, $email, $phonenumber, $hours, $address, $compensation)) {
