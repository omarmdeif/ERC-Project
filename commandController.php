...
$name = $email = $pNumber = $hours = $address = $compensation = "";
$name_err = $email_err = $pNumber_err = $hours_err = $address_err = $compensation_err = "";
...
if (isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (empty($name_err) && empty($email_err) && empty($pNumber_err) && empty($hours_err) && empty($address_err)  && empty($compensation_err)) {
        include_once '../Model/updateClass.php';
        $updator = new updateClass("volunteers");
        if ($updator->update($name, $email, $pNumber, $hours, $address, $compensation, $id)) {
